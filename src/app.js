...
const S = {
  ...
  audioBuf: null, audioName: '', htmlName: '', audioFile: null, outHandle: null, outBlob: null,
...
async function trialRun(profile, bitrate, fps, frames) {
  ...
  const at = i => segs().grand * (i + 0.5) / frames;

  let s = performance.now();
  let pending = drawTimelineFrame(at(0), ctxs[0], 1, S.mode);
  ...
  for (let f = 0; f < frames && !err; f++) {
    ...
    if (f + 1 < frames) {
      s = performance.now();
      nextPending = drawTimelineFrame(at(f + 1), ctxs[1 - cur], 1, S.mode);
      T.compose += performance.now() - s;
    }
...
/* The shell's own curve, reimplemented here because calling across the frame
   ...
   Boundaries are passed in rather than read from CHAPTERS, because with an
   intro or outro attached the fade points are the shifted chapter starts plus
   the two joins. A join is treated as exactly a chapter boundary — same curve,
   same four duration fields. Special-casing it would mean two fade behaviours
   to reason about instead of one.

   The shell's envelope is symmetric: one duration governs both the dip into a
   boundary and the recovery out of it. This takes the two separately, so a 1s
   fade-in and a 0.3s fade-out are expressible. Equal values reproduce the
   shell exactly, which is what verifyEnvelope() tests. */
function makeEnvelopeB(bounds, total) {
  return function (t, fadeIn, fadeOut) {
    if (fadeOut === undefined) fadeOut = fadeIn;
    if (!(fadeIn > 0) && !(fadeOut > 0)) return 1;
    let e = 1;
    for (const bd of bounds) {
      if (bd <= 0 || bd >= total) continue;
      if (t < bd) {
        if (fadeOut > 0 && bd - t < fadeOut) e = Math.min(e, (bd - t) / fadeOut);
      } else {
        if (fadeIn > 0 && t - bd < fadeIn) e = Math.min(e, (t - bd) / fadeIn);
      }
    }
...
function makeEnvelope(CHAPTERS, TOTAL) {
  return makeEnvelopeB((CHAPTERS || []).map(c => c.start), TOTAL);
}

/* Every fade point on the assembled timeline: chapter starts pushed right by
   the intro, plus the intro->episode and episode->outro joins. */
function timelineBounds() {
  const g = segs();
  const out = (bridgeChapters()).map(c => c.start + g.i);
  if (g.i > 0) out.push(g.i);
  if (g.o > 0) out.push(g.i + g.ep);
  /* Chapter 1 starts exactly at the intro join, so without this the same
     boundary is applied twice. Harmless to the maths, confusing in a log. */
  return [...new Set(out.map(x => Math.round(x * 1e6) / 1e6))].sort((a, b) => a - b);
}

function makeTimelineEnvelope() { return makeEnvelopeB(timelineBounds(), segs().grand); }
...
/* Decode at a chosen sample rate. Chrome resamples inside decodeAudioData, so
   an intro at 44.1k and a mix at 48k land on one grid without a resampler
   here. Mixing rates by hand is how pitch bugs get shipped. */
async function decodeAudioAt(file, sampleRate) {
  const ac = new (window.AudioContext || window.webkitAudioContext)({ sampleRate: sampleRate });
  try { return await ac.decodeAudioData(await file.arrayBuffer()); }
  finally { try { await ac.close(); } catch (e) {} }
}

/* Builds the ducked track for the whole assembled timeline: intro, episode and
   outro laid onto one buffer, with the shell's chapter envelope applied as
   gain across all of it. This replaces the ffmpeg -af string the shell used to
   hand over, and comes from the same curve the picture uses rather than a
   second implementation that can drift from it.

   parts: [{ buf, at, tail }] — tail is where that part's own audio stops, if
   it stops before its slot ends (an episode whose mix ends under an outro
   card). Everything else is governed by the shared envelope. */
function buildAudioParts(parts, grand, env, duckIn, duckOut, gain, sr) {
  const chs = parts.reduce((m, p) => Math.max(m, Math.min(2, p.buf.numberOfChannels)), 1);
  const outLen = Math.ceil(grand * sr);
  const out = [];
  for (let c = 0; c < chs; c++) out.push(new Float32Array(outLen));

  for (const part of parts) {
    const off = Math.round(part.at * sr);
    const n = Math.min(part.buf.length, outLen - off);
    for (let c = 0; c < chs; c++) {
      const src = part.buf.getChannelData(Math.min(c, part.buf.numberOfChannels - 1));
      const dst = out[c];
      for (let i = 0; i < n; i++) {
        const t = (off + i) / sr;
        let g = env(t, duckIn, duckOut);
        if (part.tail != null && duckOut > 0 && part.tail - t < duckOut) {
          g = Math.min(g, Math.max(0, (part.tail - t) / duckOut));
        }
        dst[off + i] += src[i] * g * gain;
      }
    }
  }
  /* Parts never overlap, so summing cannot clip on its own — but a hot mix
     plus a hot sting can still exceed unity, so clamp rather than wrap. */
  for (let c = 0; c < chs; c++) {
    const d = out[c];
    for (let i = 0; i < d.length; i++) { if (d[i] > 1) d[i] = 1; else if (d[i] < -1) d[i] = -1; }
  }
...
const PV = {
  ...
  el: null, elSrc: null, elUrl: '', seg: 'ep',
};

function pvTotal() { return segs().grand; }
function pvChapters() { return bridgeChapters(); }
...
function pvEnv() {
  if (!PV._env) PV._env = makeTimelineEnvelope();
  return PV._env;
}
...
/* Preview audio runs through an <audio> element rather than an
   AudioBufferSourceNode. A buffer source's playbackRate resamples, so 2x
   playback lifts the voice an octave and KateAsta turns into a chipmunk —
   useless for judging whether a beat lands on the right word. HTMLMediaElement
   has preservesPitch, which time-stretches instead, so the pitch holds at every
   speed. It is also a better clock: currentTime is the decoder's own position,
   not arithmetic layered on top of it.

   createMediaElementSource may only be called once per element, so the element
   and its graph are built when the file loads and reused for the session. */
function pvAttachAudio(file) {
  pvDetachAudio();
  const ctx = pvAudioCtx();
  PV.elUrl = URL.createObjectURL(file);
  const el = new Audio();
  el.src = PV.elUrl;
  el.preload = 'auto';
  ...
  PV.el = el;
  try {
    PV.elSrc = ctx.createMediaElementSource(el);
    PV.elSrc.connect(PV.gain);
  } catch (e) {
    log('Could not route preview audio through the gain node — duck preview disabled.', 'warn');
    PV.elSrc = null;
  }
}

function pvDetachAudio() {
  if (PV.el) { try { PV.el.pause(); } catch (e) {} }
  if (PV.elSrc) { try { PV.elSrc.disconnect(); } catch (e) {} PV.elSrc = null; }
  if (PV.elUrl) { URL.revokeObjectURL(PV.elUrl); PV.elUrl = ''; }
  PV.el = null;
}

/* Preview audio follows the timeline: the intro's own element while the intro
   plays, the mix during the episode, the outro's after. Each is already routed
   through the same gain node, so the duck applies across all three. */
function pvActive() {
  const seg = segAt(PV.t);
  if (seg.kind === 'ep') return PV.el ? { el: PV.el, local: seg.local } : null;
  return (seg.att && seg.att.el) ? { el: seg.att.el, local: seg.local } : null;
}

function pvStopAudio() {
  [PV.el, ATT.intro && ATT.intro.el, ATT.outro && ATT.outro.el].forEach(el => {
...
function pvStartAudio() {
  ...
  const act = pvActive();
  /* Pause whatever else was sounding; segments never overlap. */
  [PV.el, ATT.intro && ATT.intro.el, ATT.outro && ATT.outro.el].forEach(el => {
    if (el && (!act || el !== act.el)) { try { el.pause(); } catch (e) {} }
  });
  if (!act) return;
  if (act.local >= (act.el.duration || Infinity)) return;   // no audio left in this piece
  try {
    act.el.playbackRate = PV.speed;
    act.el.currentTime = act.local;
    const pr = act.el.play();
...
function pvGoto(t) {
  ...
  PV.anchorWall = performance.now();
  PV.seg = segAt(PV.t).kind;
  if (PV.playing) pvStartAudio();
  else {
    pvStopAudio();
    const act = pvActive();
    if (act) { try { act.el.currentTime = Math.min(act.local, (act.el.duration || 0)); } catch (e) {} }
  }
  pvPaint();
  pvSyncUI();
}
...
function pvChapterStep(dir) {
  const g = segs();
  const ch = pvChapters().map(c => ({ start: c.start + g.i }));
  if (!ch.length) return;
  let idx = 0;
...
function pvSetSpeed() {
  ...
  /* A media element takes a rate change in place, pitch preserved — no restart,
     no gap. The buffer-source version had to tear down and re-seek. */
  [PV.el, ATT.intro && ATT.intro.el, ATT.outro && ATT.outro.el].forEach(el => {
    if (el) el.playbackRate = PV.speed;
  });
  PV.anchorWall = performance.now();
}
...
async function pvDraw() {
  ...
  try {
    ...
    await drawTimelineFrame(PV.t, PV.cx, op, S.mode);
...
function pvGain() {
  ...
  const seg = segAt(PV.t);
  if (seg.kind === 'ep' && S.audioBuf) {
    const g0 = segs();
    const end = g0.i + S.audioBuf.duration;
    if (f.aOut > 0 && end < g0.i + g0.ep && end - PV.t < f.aOut) {
      g = Math.min(g, Math.max(0, (end - PV.t) / f.aOut));
    }
  }
...
function pvTick() {
  ...
  const total = pvTotal();
  const act = pvActive();
  if (act && !act.el.paused) {
    PV.t = PV.t - segAt(PV.t).local + act.el.currentTime;
  } else {
  ...
  /* Two cases hand the clock back to the wall: an episode whose mix ends under
     an outro card, and a crossing into a piece with different audio. */
  const seg2 = segAt(PV.t);
  if (seg2.kind !== PV.seg) { PV.seg = seg2.kind; PV.anchorWall = performance.now(); pvStartAudio(); }
  const act2 = pvActive();
  if (act2 && !act2.el.paused && segAt(PV.t).local >= (act2.el.duration || Infinity) - 0.05) {
...
function pvSyncUI() {
  ...
  const ch = pvChapters();
  let idx = -1;
  if (seg.kind === 'ep') for (let i = 0; i < ch.length; i++) if (ch[i].start <= seg.local + 1e-3) idx = i;
  const btns = $('pvChaps').children;
  for (let i = 0; i < btns.length; i++) btns[i].classList.toggle('on', i === idx);
  $('pvGoIntro').classList.toggle('on', seg.kind === 'intro');
  $('pvGoOutro').classList.toggle('on', seg.kind === 'outro');
}
...
function pvBuild() {
  ...
  pvChapters().forEach((c, i) => {
    ...
    b.className = 'chbtn';
    b.textContent = (i + 1);
    b.title = (c.title || ('Chapter ' + (i + 1))) + ' · ' + fmt(c.start + g.i);
    b.onclick = () => pvGoto(c.start + g.i);
    wrap.appendChild(b);
  });
  $('pvGoIntro').disabled = !ATT.intro;
  $('pvGoOutro').disabled = !ATT.outro;
  $('pvGoIntro').title = ATT.intro ? 'intro · ' + fmt(g.i) : 'no intro attached';
  $('pvGoOutro').title = ATT.outro ? 'outro · ' + fmt(g.o) : 'no outro attached';
...
/* Intro and outro are whole media files spliced onto either end of the
   episode. They may carry video, audio, or both — an MP3 sting and a branded
   WebM card are both valid, and neither is required.

   Frames come out of a <video> element seeked frame by frame rather than
   played. Playing would reintroduce exactly the wall-clock dependency that
   made MediaRecorder produce a 53-minute file; seeking is deterministic, and
   an intro is a few hundred frames, so the cost is irrelevant.

   This app has muxers but no demuxers, and vendoring mp4box.js to decode a
   five-second sting would be a poor trade. Seeking works for any container
   Chrome can already play, which is the same set the person is choosing from. */
const ATT = { intro: null, outro: null };

function bridgeChapters() { try { return bridge().CHAPTERS || []; } catch (e) { return []; } }
function episodeTotal() { try { return bridge().TOTAL || 0; } catch (e) { return 0; } }

function segs() {
  const i = ATT.intro ? ATT.intro.dur : 0;
  const o = ATT.outro ? ATT.outro.dur : 0;
  const ep = episodeTotal();
  return { i: i, ep: ep, o: o, grand: i + ep + o };
}

/* Which piece of the assembled timeline time T falls in, and where inside it. */
function segAt(T) {
  const g = segs();
  if (g.i > 0 && T < g.i) return { kind: 'intro', local: T, att: ATT.intro };
  if (T < g.i + g.ep) return { kind: 'ep', local: T - g.i };
  return { kind: 'outro', local: Math.min(Math.max(0, T - g.i - g.ep), g.o), att: ATT.outro };
}

async function loadAttachment(kind, file) {
  clearAttachment(kind);
  const url = URL.createObjectURL(file);
  const el = document.createElement('video');
  el.src = url; el.preload = 'auto'; el.playsInline = true; el.muted = false;
  try {
    await new Promise((res, rej) => {
      el.onloadedmetadata = res;
      el.onerror = () => rej(new Error('the browser cannot read this file'));
      setTimeout(() => rej(new Error('timed out reading metadata')), 20000);
    });
  } catch (e) {
    URL.revokeObjectURL(url);
    log(`${kind}: ${e.message}. Use a format Chrome can play — MP4, WebM, MP3, WAV.`, 'err');
    return false;
  }
  if (!isFinite(el.duration) || el.duration <= 0) {
    URL.revokeObjectURL(url);
    log(`${kind}: no usable duration in ${file.name}.`, 'err');
    return false;
  }
  const a = { file: file, url: url, el: el, dur: el.duration,
              hasVideo: el.videoWidth > 0, name: file.name, src: null };
  /* Route it through the preview gain node so the duck applies here too. */
  try {
    const ctx = pvAudioCtx();
    a.src = ctx.createMediaElementSource(el);
    a.src.connect(PV.gain);
  } catch (e) { a.src = null; }
  ATT[kind] = a;
  log(`${kind}: ${file.name} — ${fmt(a.dur)} · ${a.hasVideo ? el.videoWidth + '×' + el.videoHeight + ' video' : 'audio only'}`, 'ok');
  if (!a.hasVideo) log(`  no video track, so the ${kind === 'intro' ? 'first' : 'last'} episode frame is held underneath it.`);
  return true;
}
...
function seekMedia(el, t) {
  return new Promise((res) => {
    ...
    let done = false;
    const ok = () => { if (!done) { done = true; el.removeEventListener('seeked', ok); res(); } };
    el.addEventListener('seeked', ok);
    /* A seek past the last keyframe can simply never fire. Take the frame we
       have rather than stalling the whole render on it. */
    setTimeout(ok, 900);
    try { el.currentTime = Math.max(0, Math.min(t, (el.duration || 0) - 1e-3)); } catch (e) { ok(); }
  });
}

/* Contain-fit onto the brand canvas: an intro shot at a different aspect gets
   letterboxed in cream rather than stretched. */
function drawContain(ctx, src, w, h, opacity) {
  ctx.fillStyle = '#E4DDD0';
  ctx.fillRect(0, 0, 1920, 1080);
  if (!w || !h) return;
  const sc = Math.min(1920 / w, 1080 / h);
...
/* One frame of the assembled timeline, whichever piece it belongs to. */
async function drawTimelineFrame(T, ctx, opacity, mode) {
  const seg = segAt(T);
  if (seg.kind === 'ep' || !seg.att) {
    await decodeToCanvas(composeFrame(Math.max(0, Math.min(seg.local, episodeTotal())), opacity, mode), ctx);
    return;
  }
  if (seg.att.hasVideo) {
    await seekMedia(seg.att.el, seg.local);
    drawContain(ctx, seg.att.el, seg.att.el.videoWidth, seg.att.el.videoHeight, opacity);
  } else {
    /* Audio-only attachment: hold the adjacent episode frame so the screen is
       never blank under a sting. */
    const hold = seg.kind === 'intro' ? 0 : episodeTotal();
    await decodeToCanvas(composeFrame(hold, opacity, mode), ctx);
  }
}
...
async function render() {
  ...
  S.outHandle = null; S.outBlob = null;
  ...
  try {
    ...
    const env = makeTimelineEnvelope();
    const chk = (G.i || G.o)
      ? { ok: false, spliced: true }
      : verifyEnvelope(env, fadeVIn, fadeVOut);
    if (G.i || G.o) {
      log(`Timeline — ${G.i ? 'intro ' + fmt(G.i) + ' + ' : ''}episode ${fmt(G.ep)}${G.o ? ' + outro ' + fmt(G.o) : ''} = ${fmt(G.grand)}`, 'ok');
    }
    if (chk.ok) log('Fade curve matches the shell exactly', 'ok');
    else if (chk.spliced) log('Timeline is spliced, so the shell has nothing to compare against. Joins fade exactly like chapter boundaries.');
    ...
    if (withAudio) {
      ...
      const epAudioDur = (typeof B.EPISODE.audioDur === 'number') ? B.EPISODE.audioDur : G.ep;
      const parts = [{ buf: S.audioBuf, at: G.i, tail: (epAudioDur < G.ep) ? G.i + epAudioDur : null }];
      for (const k of ['intro', 'outro']) {
        const a = ATT[k]; if (!a) continue;
        try {
          const buf = await decodeAudioAt(a.file, sr);
          parts.push({ buf: buf, at: k === 'intro' ? 0 : G.i + G.ep, tail: null });
        } catch (e) {
          log(`${k} carries no decodable audio — its picture is used, silently.`, 'warn');
        }
      }
      parts.sort((x, y) => x.at - y.at);
      const track = buildAudioParts(parts, TOTAL, env, duckIn, duckOut, +$('vol').value / 100, sr);
    ...
    let pending = drawTimelineFrame(0, ctxs[0], opacityAt(0), S.mode);
    ...
    for (let f = 0; f < totalFrames; f++) {
      ...
      if (f + 1 < totalFrames) {
        const tn = (f + 1) / fps;
        s0 = performance.now();
        nextPending = drawTimelineFrame(tn, ctxs[1 - cur], opacityAt(tn), S.mode);
        composeMs += performance.now() - s0;
      }
...
/* A web page cannot launch VLC — starting a local application from a browser
   tab is a security boundary with no workaround, and anything claiming
   otherwise is a browser extension. What it CAN do is hand the finished file
   to a new tab, where Chrome's own player handles H.264/AAC and VP9/Opus
   natively. Reading it back through getFile() gives a File backed by disk, so
   opening a gigabyte master does not pull a gigabyte into memory.

   For the real thing — VLC, Premiere, YouTube upload — the file is already
   saved where the picker put it. */
async function openResult() {
  try {
    let url;
    if (S.outHandle) {
      const f = await S.outHandle.getFile();
      url = URL.createObjectURL(f);
    } else if (S.outBlob) {
      url = URL.createObjectURL(S.outBlob);
    } else { log('Nothing to open.', 'warn'); return; }
    const w = window.open(url, '_blank');
    if (!w) log('The browser blocked the new tab — allow pop-ups for this page, or open the saved file directly.', 'warn');
    setTimeout(() => URL.revokeObjectURL(url), 120000);
  } catch (e) {
    log('Could not open the file: ' + e.message + '. It is still saved where you chose.', 'warn');
  }
}

function showOpen(name) {
  const b = $('bOpen');
  b.style.display = '';
  b.title = 'play ' + name + ' in a new tab';
}
...
ready(() => {
  ...
  $('fAudio').onchange = async e => {
    const f = e.target.files[0]; if (!f) return;
    try {
      S.audioBuf = await decodeAudio(f); S.audioName = f.name; S.audioFile = f;
      pvAttachAudio(f);
  ...
  pvWire();
  $('bOpen').onclick = openResult;
  $('pvGoIntro').onclick = () => pvGoto(0);
  $('pvGoOutro').onclick = () => { const g = segs(); pvGoto(g.i + g.ep); };

  for (const kind of ['intro', 'outro']) {
    $('f' + kind[0].toUpperCase() + kind.slice(1)).onchange = async e => {
      const f = e.target.files[0]; if (!f) return;
      if (await loadAttachment(kind, f)) {
        const g = segs();
        log(`Timeline is now ${fmt(g.grand)} — ${g.i ? fmt(g.i) + ' + ' : ''}${fmt(g.ep)}${g.o ? ' + ' + fmt(g.o) : ''}`);
        if (S.win) pvBuild();
      }
    };
  }

  /* A render is minutes of work that cannot be resumed, and a stray Ctrl+R
     throws it away silently. The browser shows its own generic wording; all a
     page can do is ask for the prompt. */
  window.addEventListener('beforeunload', e => {
    if (!S.running && !S.win) return;
    e.preventDefault();
    e.returnValue = '';
    return '';
  });
